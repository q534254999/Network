...
(slideViewDidLoad = function(_container,_id){
	if(_id == 1) {
		...
		// For iPad & iPhone & mobile device width less than 768 , fixing CSS to align text properly within cell
    	if(window.navigator.userAgent.match('iPad') || window.navigator.userAgent.match('iPhone') || (window.innerWidth <= 768)){
    		setTimeout(function(){
             var _q4Text = $('#txt_q4').css('top','7px');
            },200);
    	} 
    }
	if(_id==2)
	...
	function removeAns(){
		...
		{
            //console.log($('#'+th.elements.texts[i].compId)+' :::: '+$('#'+th.elements.texts[i].compId).length);
			if( $('#'+th.elements.texts[i].compId).length > 0){
                $('#'+th.elements.texts[i].compId).remove(); 
                //alert("removed:"+ th.elements.texts.length);
           }
            
            $('#ID_s2_ans0'+(i+1)).remove();		
		}
        
        
    
        
      // console.log('final length: '+th.elements.texts.length);
