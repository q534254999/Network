...
(slideViewDidLoad = function(_container,_id)
 {	
	...
	feeback = getCJSElement("TEXTBOX",{subType:"CLOSABLE",x:322.3,y:298.75,width:240,height:160,titleId:"ID_MsgBadTitle",bodyId:"ID_MsgBadBody",visible:"0"});			  
	_container.addChild(feeback);
	
	feeback2 = getCJSElement("TEXTBOX",{subType:"CLOSABLE",x:22.3,y:298.75,width:240,height:160,titleId:"ID_MsgGoodTitle",bodyId:"ID_MsgGoodBody",visible:"0"});			  
	_container.addChild(feeback2); 

	//images...
		
	pass_btn = getCJSElement("BLUE_BUTTON",{x:722,y:158.75,width:108,height:21,size:12,id:"pass",compId:"ID_btn01",callback:toggleBlock});
	_container.addChild(pass_btn);
		
	block_btn = getCJSElement("BLUE_BUTTON",{x:722,y:186.65,width:108,height:21,size:12,id:"block",compId:"ID_btn02",callback:toggleBlock});
	_container.addChild(block_btn);

	start_btn = getCJSElement("BLUE_BUTTON",{x:706.5,y:435.1,width:108,height:21,size:12,id:"new_nub2",compId:"ID_btn03",callback:startGame});
	_container.addChild(start_btn);
		
	reset_btn = getCJSElement("BLUE_BUTTON",{x:822.75,y:435.1,width:108,height:21,size:12,id:"new_nub",compId:"ID_btn04",callback:resetGame});
			...
			if (!(parseInt($("#ID_score").html()) == 10 || parseInt($("#ID_score").html()) == 0)) {
				if(packetCont.x < maxX){
					...
					if(packetCont.x > maxX - (access == "none"?10:120)){
						scoreCont.alpha = 1;
						if(isFirst){
							createjs.Tween.get(scoreCont).to({y:75}, 1000, createjs.Ease.linear).wait(1000).call(updatePacket);
							isFirst = false;
							count++;
							if(correctAnswer == access){
