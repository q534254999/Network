...
(FIBActivity = function(container,id,answerArr){
	...
	{				
		...
		$("#iTxt0"+id+"_0"+i).keyup(function (event) {
			this.value = this.value.replace(/[^0123456789\.ABCDEF]/g,'');
		});
		
		var _domBitIn = new createjs.DOMElement(document.getElementById(bitIn.id));
		_domBitIn.x = i*49.7+510 ;
		_domBitIn.y = 184;				
	...
	bitIn.data = 8;
	bitIn.style.cssText = "position:absolute;width:320px;text-align:left;top:0px;left:1px";
	bitIn.onchange = function(event) {
		validateThis(this.value,this.data,this.id);
	}
	$("#commonMediaText").append(bitIn);
	$("#iTxt0"+id+"_08").keyup(function (event) {
