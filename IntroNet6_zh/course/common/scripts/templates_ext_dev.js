...
(DragDropComp = function(obj, xmlNode) {
    ...
    var snabBackAnswer = false;
    var showSnapFBComp = false;
    //var droptarget
    ...
    th.mouseUpHandler = function(ev) {
            ...
            for (var i = 0; i < dropArray.length; i++) {
                ...
                if (mediaStage.mouseInBounds && ev.target.hitTest(dropPoint.x, dropPoint.y)) {
                    ...
                    //alert(obj.method+" != SNAPBACK || "+dropArray[i].obj.group+' == '+dragArray[dragid].obj.group);
                    /*if(obj.method == "SNAPBACK") {
                    	//checkFirst = 1;
                    	if (ev.target.dropTarget) ev.target.dropTarget.clear();

                    	th.buttonClickHandler("check");
                        //alert(snabBackAnswer);
                    	if(snabBackAnswer) {
                        	isHit = true;
                        	//ev.target.removeEventListener('mouseDownHandler');
                    		...
                    		isHit = false;
                            //ev.target.addEventListener('mouseDownHandler');
                            //ev.target.addEventListener("mousedown", th.mouseDownHandler);
                             //ev.target.addEventListener("mousemove", th.mouseMoveHandler);
                                //ev.target.addEventListener("mouseup", th.mouseUpHandler);
                ...
                //console.log(snpbackcnt+" == "+dragArray.length+' && '+obj.method+" == SNAPBACK");
                ...
                if(obj.method == "SNAPBACK" && showSnapFBComp) {
                	fbComp.visible = true;
                    fbComp.setTitleId(fbObject.correctTitleId);
                    fbComp.setBodyId(fbObject.correctBodyId);
                    snapbackcomplete = 1;
    ...
    th.buttonClickHandler = function(id) {
        ...
        if (id == "check") {
            ...
            for (m = 0; m < dropArray.length; m++) {
                ...
                //alert('correctanswercount: '+correctanswercount);
                ...
                if (dropArray[i].dragTarget) {
                    ...
                    if (obj.matchType == "group") {
                        ...
                        if (dragId == dropArray[i].obj.match) {
                        	...
                        	console.log('coming to ubnormal');
                        	snabBackAnswer = false;
                        }
                    }
                }
           ...
           // alert(obj.method);
            if(obj.method == "group") {
                //alert('coming here');
                //alert(cnt+' == '+totaldragTarget+' && '+totaldragTarget+' == '+dragArray.length);
                //alert(cnt+' == '+totaldragTarget);
                if (cnt == totaldragTarget && totaldragTarget == dragArray.length) { //console.log("show correct");    
                    fbComp.setTitleId(fbObject.correctTitleId);
                    fbComp.setBodyId(fbObject.correctBodyId);
                } else { //console.log("show incorrect");           
                    fbComp.setTitleId(fbObject.wrongTitleId);
                    fbComp.setBodyId(fbObject.wrongBodyId);
                }
                fbComp.visible = true;
            } else if(obj.method != "SNAPBACK") {
            	//alert('coming here');
                //alert(cnt+' == '+correctanswercount+' && '+totaldragTarget+' == '+correctanswercount);
            	if (cnt == correctanswercount && totaldragTarget == correctanswercount) { //console.log("show correct");	
	                fbComp.setTitleId(fbObject.correctTitleId);
	                fbComp.setBodyId(fbObject.correctBodyId);
	            } else { //console.log("show incorrect");			
	                fbComp.setTitleId(fbObject.wrongTitleId);
    ...
    $(xmlNode).find('subelement').each(function() {
        ...
        switch (type) {
            ...
            case "FEEDBACK":
                fbObject = compObj;
                cjsElement = getCJSElement("TEXTBOX", { subType: "CLOSABLE", x: parseFloat(compObj.x), y: parseFloat(compObj.y), width: parseFloat(compObj.width), height: parseFloat(compObj.height), titleId: compObj.correctTitleId, bodyId: compObj.correctBodyId, visible: "0" });
                fbComp = cjsElement;
                break;
            case "INSTRUCTION":
                compObj.div = obj.div;
...
(McTable = function(o, xmlNode) {
    ...
    var loaded = function(_obj) {
        ...
        for (var k = parseInt(tableCols); k > 0; k--) {
            ...
            if (k == tableCols) { styleHead = "border:none;";
                styleHeadLast = "" } else if (k == 1) { styleHead = "border-right:2px solid #fff;"; } else { styleHead = "border-right:2px solid #fff;"; }
    ...
    $(xmlNode).find('subelement').each(function() {
        ...
        switch (type) {
            ...
            case "RESET":
                ...
                break;
            case "FEEDBACK":
            	//alert('compObj.height: '+compObj.height);
                fbObject = compObj;
