...
{	
	...
	var xVal1=145,y1 = 108.4,	width=340,height=295,inital_height=295,br=0;
	var initial_y1=108.4;
//drawRect(legendObj.color,Number(legendObj.x),Number(legendObj.y),Number(legendObj.width),Number(legendObj.height),Number(legendObj.br));
	...
	topCont=  drawRect("#33abdf",140,60,420,28.4,10); 
    ...
    bottomCont = drawRect("#0197d6",140,398.2,420,28.4,10);
	bottomCont.name="bottomCont";
	bottomCont.cursor="pointer";		
	slideContainer.addChild(bottomCont);
	
   drawRect("#cccccc",150,78.4,410,330,0);//right side grey shape	
	

	drawRect("#4E4C4D",140,78.4,350,330.2,0);//black rectangle
	...
	var tlManifest = [{id:"TimelineLayoutXml", src:'media_3.2.2.2_layout.xml'},
	...
	{			
		...
		/*	roundBtn.imgX=obj.imgX;
			...
			$("#commonMediaText").append("<div id='slide_1_"+index+"' class='slideMove'></div>");
	...
	function enableContainer(cont,bool)
	{
		if(bool)
		{
			...
			{
				cont.shape.graphics.beginFill("#0197d6").drawRoundRect(140,60,420,28.4,10);
				
			}
			else
			{
				bottomCont.shape.graphics.beginFill("#0197d6").drawRoundRect(140,388.2,420,28.4,10);				
			}
		}
		else
		{
			...
			if(cont.name=="topCont")
			{
				cont.shape.graphics.beginFill("#33abdf").drawRoundRect(140,60,420,28.4,10);
				
			}
			else
			{
				bottomCont.shape.graphics.beginFill("#33abdf").drawRoundRect(140,388.2,420,28.4,10);				
			}
		}
