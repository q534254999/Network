...
(ClickActivity = function(obj,xmlNode){
	...
	this.createElement = function(compObj) {
		...
		bgShape.graphics.beginFill("#FFFFFF").drawRect(compObj.x ,compObj.y,obj.width,obj.height); 
		...
		$(xmlNode).find('subelement').each(function(){
			...
			switch(type){				
					case "BTN": 
						...
						compObj.x = $(this).attr('x') ? $(this).attr('x') : 10;		
					...
					case "BLUE_BUTTON":  
						compObj.compId = $(this).attr('compId') ? $(this).attr('compId') : "ID_s1_btn01";	
						compObj.width = $(this).attr('width') ? $(this).attr('width') : 60;	
